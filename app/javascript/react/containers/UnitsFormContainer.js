...
class UnitsFormContainer extends Component {
	constructor(props) {
		...
		this.clearArmyErrors = this.clearArmyErrors.bind(this)
		...
		this.validateArmy = this.validateArmy.bind(this)
	...
	clearArmyErrors(event) {
		let errorState = this.state.errors
		delete errorState.army
		this.setState({ errors: errorState })
	}
	...
	validateArmy(input) {
		if (input === '') {
			let newError = {
				army: 'You must select the unit\'s army'
			}
			this.setState({
				errors: Object.assign(this.state.errors, newError)
			})
	...
	updateUnitSize(event) {
		...
		if (
			this.state.unitSize.trim() !== '' ||
			this.state.unitSize.length > 0
		) {
			this.clearUnitSizeErrors()
		}
	}
	...
	submitForm(event) {
		...
		if (this.validateArmy(this.state.army)) {
			dataToAdd.army_id = this.state.army
		}
		if (
			dataToAdd.name != '' &&
			dataToAdd.unit_size != '' &&
			...
			<div>
				<form onSubmit={this.submitForm}>
					...
					<div className="unit-army-dropdown">
						<UnitArmyDropdown
							selection={this.state.army}
							label="Army: "
							name="army"
							handlerFunction={this.updateArmy}
						/>
					</div>
